...
- `UOnsetVoipSoundWave` no longer derives from `USoundWaveProcedural`. It now uses Unreal's `USoundWave`/`ISoundGenerator` path, so custom code that casts it to `USoundWaveProcedural`, calls `QueueAudio()`, binds procedural underflow delegates, or passes raw int16 byte buffers to `AddAudioData()` must be updated to use the Onset VoIP float PCM APIs instead.
