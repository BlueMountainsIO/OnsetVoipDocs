...
The plugin implements the logging category named `LogOnsetVoip`. By default, `LogOnsetVoip` logs messages at `Log` level and above. To enable logging of lower-level functionality like sending packets you can set the verbosity to `Verbose` or `VeryVerbose`.
...
To have the logging verbosity saved between editor restarts or in development builds you can configure them in your `DefaultEngine.ini`:
